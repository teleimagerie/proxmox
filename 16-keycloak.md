...
| **Vaultwarden** (`vault.teleimagerie.net`) | SSO OIDC natif (≥ 1.35), `SSO_ONLY` — pas de login local, client `vaultwarden` du realm `tim` (18/09/2026) | aucun — création à la volée au premier login SSO (e-mail vérifié, `SIGNUPS_DOMAINS_WHITELIST=teleimagerie.net`), master password défini par l'utilisateur | aucun login possible, mais les clients connectés déverrouillent leur cache local au master password |
...
|---|---|
| `proxmox` | `https://pve{1..5}.infra.teleimagerie.net:8006/*` (`pve4`/`pve5` ajoutés le 15/09/2026 par `kcadm update clients/<id> -r tim -s 'redirectUris=[…]'`, après `kcadm.sh config credentials` — la session kcadm du CT 203 expire, le mot de passe admin ne vit que dans le gestionnaire de secrets) |
...
| `vaultwarden` (créé le 18/09/2026, PKCE S256 imposé, **Access Token Lifespan 600 s au niveau du client** — le défaut realm de 5 min déconnecte les clients Bitwarden — et scope optionnel `offline_access` demandé) | `https://vault.teleimagerie.net/identity/connect/oidc-signin` |
...
| **Vaultwarden** (`vault.teleimagerie.net`, coffre de mots de passe) | OIDC (support natif ≥ 1.35, **pas de SAML**) | ✅ **déployé le 18/09/2026** ([21-vaultwarden.md](21-vaultwarden.md)) : VM 105, client `vaultwarden` (tableau ci-dessus), `SSO_ONLY` — identités Google via le broker existant, rien touché côté Google |
