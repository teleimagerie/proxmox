...
| **Vaultwarden** (`vault.teleimagerie.net`, coffre de mots de passe) | OIDC (support natif ≥ 1.35, **pas de SAML**) | 📋 **dossier de déploiement préparé le 18/09/2026** ([21-vaultwarden.md](21-vaultwarden.md)) : client `vaultwarden` à créer (patron habituel + `access.token.lifespan=600` côté client, scope `offline_access` demandé), `SSO_ONLY` — identités Google via le broker existant, rien à toucher côté Google |
