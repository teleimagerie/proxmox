...
| Plantages | **3 à 17 vidages par jour (56 le 08/09)** depuis au moins le 12/08 : `svstream.exe` (streaming, `conn.dll` et corruption de tas), **`svdser.exe` le serveur DICOM (5 fois le 10/09)**, `svar`, `svfload`, `w3wp`. Le PACS Restarter relance. **Précision Philips du 14/09/2026** : les vidages de `svdser.exe` sont normaux, le processus se termine de lui-même après quelques minutes sans activité — l'alerte High posée le 11/09 a été retirée ; les autres processus restent à clarifier. `Watchdog_SvMax` est cassé (exécutable invalide), Mirth est muet sur 8014 (CRITICAL du contrôle interne), 19 864 études sans ordre RIS |
...
- [ ] **plantages `svstream.exe`** (3 à 56 par jour, audit du 11/09 ;
      `svdser.exe` exclu : normal selon Philips le 14/09, auto-terminaison
      après inactivité), `Watchdog_SvMax` cassé, Mirth muet sur 8014 (cause connue le
