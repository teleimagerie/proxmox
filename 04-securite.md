...
| Clé API + secret Mailjet (SMTP Keycloak **+ notifications PVE/PBS**) | **gestionnaire de secrets** | même clé (nommée « keycloack » chez Mailjet, seule clé du compte). Copies opérationnelles : config du realm `tim` (base Keycloak, 30/08/2026), `/etc/pve/priv/notifications.cfg` (root-only, **répliqué** pmxcfs) et `/etc/proxmox-backup/notifications-priv.cfg` de la VM 102 (30/08/2026) ; transits par pve1/PBS détruits après test — [16-keycloak.md](16-keycloak.md#e-mail-sortant--smtp-mailjet-en-place-depuis-le-30082026) |
| Token PBS `zabbix@pbs!monitoring` (rôle Audit, lecture seule) | macro secrète `{$PBS.TOKEN.SECRET}` de l'hôte `cluster-pve` dans Zabbix | créé le 30/08/2026 pour la supervision des sauvegardes ([17-zabbix.md](17-zabbix.md#supervision-des-sauvegardes--depuis-le-30082026)) — révocable par `proxmox-backup-manager user delete-token zabbix@pbs monitoring` |
