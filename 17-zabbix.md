...
| Interfaces réseau (14/09/2026) | la découverte SNMP remontait **35 à 37 « interfaces »** par serveur — commutateurs virtuels Hyper-V (`vSwitch`/`vEthernet` `nat` et `WSL`), pseudo-interfaces des pilotes de filtrage (`WFP`, `QoS Packet Scheduler`), tunnels et miniports « Local Area Connection* N » — avec **une dizaine d'Average « Link down » ouverts en permanence** ou rouverts à chaque reboot. Le filtre `{$NET.IF.IFDESCR.NOT_MATCHES}` du gabarit ne les attrape pas (Windows met le nom du pilote dans `ifDescr`) ; le libellé utile est **`ifAlias`**, celui entre parenthèses dans le nom des items. Macro d'hôte **`{$NET.IF.IFALIAS.NOT_MATCHES}`** = `vSwitch\|vEthernet\|Local Area Connection\\*\|LightWeight Filter\|QoS Packet Scheduler\|Loopback\|Hyper-V` sur les deux hôtes : il ne reste que **`HPE Network Port 10G 1`** (câblé) et **`10G 2`** (non câblé, à 2 stable : « Link down » ne sonne que sur un changement). Les items exclus sont « perdus » : désactivés aussitôt (`enabled_lifetime_type` du gabarit), supprimés après 7 jours ; leurs 70 déclencheurs « Link down » désactivés et les 10 problèmes ouverts fermés par le script. Au passage, l'erreur permanente de la règle sur `.98` (« Cannot create graph … already exists » : deux interfaces au même alias) a disparu avec elles. ⚠️ Piège : relancée dans la foulée de la macro, la découverte tourne avec l'ancien cache de configuration (10 s) et ne filtre rien — le script attend 15 s |
| Ce qu'on voit | CPU, mémoire, **tous les volumes** (dont `N:` System_Backup, déjà à plus de 90 % : le déclencheur Average du gabarit sonne, c'est voulu), les deux ports réseau HPE 10G (le reste est filtré depuis le 14/09, voir plus bas), uptime, ICMP ; ce qu'on **ne voit pas** sans agent : services Windows, journaux d'événements, compteurs de performance |
